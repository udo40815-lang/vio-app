...
function Drawer({ ui, open, onClose, setTab, setDark, onSignOut }) {
  const sections = [
    { label: 'Account', items: [
  ...
  const handleAction = (action) => {
    onClose();
    if (action === 'profile') setTab('profile');
      ...
      <div className="relative w-[300px] max-w-[80vw] h-full overflow-y-auto" style={{ background: ui.dark ? V.darkest : V.light, animation: 'vDrawerIn 280ms cubic-bezier(0.22,1,0.36,1)' }}>
        <div className="sticky top-0 z-10 px-5 pt-8 pb-4" style={{ background: ui.dark ? V.darkest : V.light }}>
          <div className="flex items-center gap-3">
            ...
            <div><div className="text-[16px] font-semibold tracking-[-0.02em]" style={{ color: ui.textPrimary }}>Vio</div><div className="text-[11px]" style={{ color: ui.textMuted }}>Where value gets discovered</div></div>
          </div>
        </div>
        <div className="mx-4 rounded-2xl p-4 flex items-center gap-3" style={{ background: ui.dark ? V.surfaceDark : '#FFFFFF', border: `1px solid ${ui.border}` }}>
          {ui.avatarUrl ? (<img src={ui.avatarUrl} alt="" className="w-10 h-10 rounded-full object-cover" />) : (<div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[14px] font-semibold" style={gradientStyle(135)}>{(ui.displayName || ui.handle || 'V').charAt(0).toUpperCase()}</div>)}
          <div className="flex-1 min-w-0"><div className="text-[14px] font-semibold truncate" style={{ color: ui.textPrimary }}>{ui.displayName || ui.handle || 'You'}</div><div className="text-[11px]" style={{ color: ui.textMuted }}>@{ui.handle || 'you'}</div></div>
            ...
            <div key={si} className="mb-5">
              ...
              <div className="rounded-2xl overflow-hidden" style={{ background: ui.dark ? V.surfaceDark : '#FFFFFF', border: `1px solid ${ui.border}` }}>
                {section.items.map((item, ii) => {
                  ...
                  return (<button key={ii} onClick={() => handleAction(item.action)} className="w-full flex items-center gap-3 px-3.5 py-3 text-left transition-colors duration-150 hover:brightness-105" style={{ color: ui.textPrimary, borderTop: ii > 0 ? `1px solid ${ui.border}` : 'none' }}><div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${V.royal}15` }}><Icon size={14} style={{ color: V.royal }} /></div><span className="text-[13.5px] font-medium flex-1">{item.label}</span><ChevronRight size={13} style={{ color: ui.textMuted }} /></button>);
                })}
              </div>
            </div>
          ))}
          <button onClick={onSignOut} className="w-full flex items-center gap-3 px-3.5 py-3 rounded-2xl text-left transition-colors" style={{ background: `${V.red}10`, border: `1px solid ${V.red}25`, color: V.red }}><div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${V.red}15` }}><LogOut size={14} /></div><span className="text-[13.5px] font-semibold">Sign out</span></button>
        </div>
      </div>
    </div>
  );
